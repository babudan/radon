...
const getBooksData= async function (req, res) {
    ...
    let allBooks = await bookModel.find().select({bookName : 1 , authorName : 1 , _id : 0 } )
     ...
     const pricetagbooks = async function (req, res) {
         let tagdata = req.body
    
        let tagSavedData = await bookModel.find({
            $or :[{"prices.indianPrice" : {$eq :"100INR"}},{"prices.indianPrice" : {$eq :"200INR"}},{"prices.indianPrice" : {$eq : "500INR"}}]}).select({bookName:1,_id:0})
         return res.send({msg: tagSavedData })
         console.log(tagSavedData)
     }
    ...
    //  const returnbooks = async function (req, res) {
    //     let yeardata = req.body.year
    //     let returnData = await bookModel.({year : Number} )
    //     res.send({msg: yearsavedData})
    //  }
